...
module.exports = {
  ...
  getDataSensorById: (deviceID, callback) => {
    db.query(
      `SELECT devices.deviceID,devices.temperature,devices.humidity,devices.soilHumidity,devices.soilPH,devices.lightIntensity FROM devices INNER JOIN farms ON farms.deviceID = ? limit 1 `,
      [deviceID],
      (err, results, fields) => {
        if (err) {
          return callback(err);
