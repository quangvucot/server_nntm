...
class DatasController {
  ...
  postSensorData(req, res) {
    ...
    try {
      ...
      getRelay((err, results) => {
        ...
        createRelay(valuesSensor, (err, results) => {
          if (err) {
            console.log(err);
            return res.status(500).send({ success: 0, message: err.message });
          }
  ...
  controllDevice(req, res, next) {
    ...
    getRelay((err, results) => {
      ...
      if (relay.relay3 == undefined) {
        relay.relay3 = results.relay3;
      }
      if (relay.relay1 == undefined) {
        relay.relay1 = results.relay1;
      }
      if (relay.relay2 == undefined) {
        relay.relay2 = results.relay2;
      }
      if (relay.relay4 == undefined) {
        relay.relay4 = results.relay4;
      }
  ...
  addControllDevice(req, res, next) {
    ...
    getRelay((err, results) => {
      ...
      createRelayForApp(body, (err, results) => {
        ...
        return res.status(200).json({ success: 1, data: body });
      });
    });
  }
